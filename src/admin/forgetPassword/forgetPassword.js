    ...
    return ( 
        <div className="forget-password">
            <form onSubmit={handleSubmit} className="forgetpassword-form">
                ...
                <TextField type="email" style={{width: '300px'}} onChange={handleInput} value={inputs.email} name='email' id="input-with-sx" label="Enter your email" variant="standard" />
                </div>

                <br />
                {error && <p style={{textAlign: 'center', color:'red'}}>{error}</p>}
                <div className="forget-password-btn">
                    <button type='submit' disabled={isSent || isSending?true:false} class="forget-password-submit-btn">{text}</button>
                </div>
                <br />
